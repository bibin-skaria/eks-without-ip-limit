...
resource "aws_subnet" "private" {
  ...
  vpc_id            = aws_vpc.main.id
  cidr_block        = cidrsubnet(var.vpc_cidr, 4, count.index + 4)  # /20 subnets, offset by 4
  availability_zone = local.azs[count.index]
...
# Pod subnets removed - using standard VPC CNI without custom networking
# Pods will get IPs from the same subnets as nodes (private subnets)
